...
class RightHandRule(Node):
    ...
    def timer_callback(self):
        ...
        self.peek_count = self.peek_count % 65
        ...
        self.get_logger().info('peek count: {}'.format(self.peek_count))
        ...
        if front_lidar_min < SAFE_STOP_DISTANCE:
            ...
            self.publisher_.publish(self.cmd)
            self.turtlebot_moving = True
            self.get_logger().info('Returning from peek')
        elif self.peek_count > 52:
            if self.gps_count >= 1:
                self.gps_count -= 1
            self.peek_count += 1
            self.cmd.linear.x = 0.0
            ...
            self.publisher_.publish(self.cmd)
            self.turtlebot_moving = False
            self.get_logger().info('Pausing to scan')
        elif self.peek_count > 45:
            if self.gps_count >= 1:
                self.gps_count -= 1
            self.peek_count += 1
            self.cmd.linear.x = 0.0
            ...
            self.publisher_.publish(self.cmd)
            self.turtlebot_moving = True
            self.get_logger().info('Peeking right')
        elif avoid_front:
            self.cmd.linear.x = 0.0 
